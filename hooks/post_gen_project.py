...
def remove_storages_module():
    """Remove the storages module."""
    storages_path = Path("{{cookiecutter.project_slug}}", "utils", "storages.py")
    if storages_path.exists():
        storages_path.unlink()


def rename_nginx_conf():
    """FORKED: We like to have the nginx conf file named `default.conf`."""
    old_name = Path("compose", "production", "nginx", "{{cookiecutter.project_slug}}.conf")
    new_name = Path("compose", "production", "nginx", "default.conf")
    if old_name.exists():
...
def remove_prometheus_compose_files():
    ...
    shutil.rmtree(Path("compose", "production", "prometheus"))


def remove_grafana_compose_files():
    """FORKED: We use Grafana."""
    shutil.rmtree(Path("compose", "production", "grafana"))


def remove_mosquitto_compose_files():
    """FORKED: We use Mosquitto."""
    shutil.rmtree(Path("compose", "production", "mosquitto"))
